...
function CreateAccount() {
  ...
  const onNewUserClick = async (e) => {
    ...
    try {
      const response = await fetch("https://apisenainotas-01-fddghxaxcna9augw.canadacentral-01.azurewebsites.net");

      if (response.ok) {
        alert("Você criou sua conta com sucesso!");
        window.location.href = "/login";
      } else {
        alert("Um erro inesperado ocorreu. Caso persista, tente novamente ou contate os administradores.");
      }
    } catch (err) {
